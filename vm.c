...
int create_shared_memory(uint size, int index)
{
  ...
  for (int i = 0; i < num_of_pages; i++)
  {
    ...
    if (new_page == 0)
    {
      cprintf("create_shared_memory: failed to allocate a page (out of memory)\n");
      release(&shmTable.lock);
      return -1;
    }
...
void *open_shared_mem(int shmid) {
  ...
  int index = shmTable.allRegions[shmid].shmid;
  if (index == -1) {
    release(&shmTable.lock);
    index = create_shared_memory(2565, shmid);
    if (index == -1) {
      return (void *)-1;
    }
    acquire(&shmTable.lock);
  ...
  // Find a valid virtual address for mapping
  struct proc *process = myproc();
  ...
  // Ensure the virtual address is within valid range
  ...
  for (int i = 0; i < SHAREDREGIONS; i++) {
    if (process->pages[i].key != -1 &&
        (uint)process->pages[i].virtualAddr + process->pages[i].size * PGSIZE > (uint)va &&
        (uint)va >= (uint)process->pages[i].virtualAddr) {
      release(&shmTable.lock);
      return (void *)-1;
    }
  }

  // Map the shared memory region to the process
  for (int k = 0; k < shmTable.allRegions[index].size; k++) {
    if (mappages(process->pgdir, (void *)((uint)va + (k * PGSIZE)), PGSIZE,
      ...
      deallocuvm(process->pgdir, (uint)va,
                 (uint)(va + shmTable.allRegions[index].size));
      release(&shmTable.lock);
      return (void *)-1;
    }
  }

  // Update process metadata for the shared memory
  for (int i = 0; i < SHAREDREGIONS; i++) {
    if (process->pages[i].key == -1) {
      process->pages[i].shmid = shmid;
      process->pages[i].virtualAddr = va;
      process->pages[i].key = shmTable.allRegions[index].key;
      process->pages[i].size = shmTable.allRegions[index].size;
  ...
  // If no available page entry, cleanup and return error
  release(&shmTable.lock);
  return (void *)-1;
}
...
void close_shared_memWrapper(void *addr)
{
